...
export function GestureTrainerOverlay({
	handResults,
	onThresholdsLearned,
}: GestureTrainerOverlayProps) {
	...
	const [samples, setSamples] = useState<Map<GestureType, GestureSample[]>>(
		new Map(),
	);
	...
	const learnThresholds = useCallback(() => {
		...
		const calcAvgCurls = (samples: GestureSample[]) => {
			...
			const averages = {
				...
				avg:
					(sums.index + sums.middle + sums.ring + sums.pinky) /
					(4 * samples.length),
			};
			...
			samples.forEach((s) => {
				variances.index += (s.fingerCurls.index - averages.index) ** 2;
				variances.middle += (s.fingerCurls.middle - averages.middle) ** 2;
				variances.ring += (s.fingerCurls.ring - averages.ring) ** 2;
				variances.pinky += (s.fingerCurls.pinky - averages.pinky) ** 2;
				variances.thumb += (s.fingerCurls.thumb - averages.thumb) ** 2;
			});
		...
		console.debug("📈 Average curls per gesture:", {
			fistAvg,
			palmAvg,
			thumbsUpAvg,
		});
		...
		const newThresholds = {
			...
			thumbsUpFingerCurl: thumbsUpAvg
				? Math.max(
						...
						thumbsUpAvg.avg -
							0.15 -
							(thumbsUpAvg.stdDevs?.avgStdDev || 0) * 0.5,
					)
				: 0.6,
			...
			thumbsUpThumbExtend: thumbsUpAvg
				? Math.min(
						0.35,
						thumbsUpAvg.thumb + 0.1 + (thumbsUpAvg.stdDevs?.thumb || 0) * 0.5,
					)
				: 0.25,
	...
	const getTotalSamples = () => {
		return Array.from(samples.values()).reduce(
		...
		return (
			<div className="absolute top-4 right-4 bg-slate-900/90 backdrop-blur-sm rounded-lg p-4 border border-purple-700/50 max-w-xs">
				<p className="text-white text-sm">
					👋 Show your hand to start training
				</p>
			</div>
		);
	}
			...
			<div className="absolute top-4 left-4 bg-slate-900/90 backdrop-blur-sm rounded-lg p-3 border border-purple-700/50 space-y-2 text-xs">
				...
				{Object.entries(fingerCurls).map(([finger, value]) => (
					<div key={finger} className="flex items-center gap-2">
						<span className="w-12 text-gray-300 capitalize text-[10px]">
							{finger}
						</span>
					...
					<>
							...
							<div className="grid grid-cols-2 gap-2">
								...
								>
									...
									<span className="text-[10px]">
										Fist ({getSampleCount(GestureType.CLOSED_FIST)}/
										{SAMPLES_NEEDED})
									</span>
								</button>
								<button
								...
								>
									...
									<span className="text-[10px]">
										Palm ({getSampleCount(GestureType.OPEN_PALM)}/
										{SAMPLES_NEEDED})
									</span>
								</button>
								<button
								...
								>
									<span className="text-xl">👍</span>
									<span className="text-[10px]">
										Thumbs ({getSampleCount(GestureType.THUMBS_UP)}/
										{SAMPLES_NEEDED})
									</span>
								</button>
								<button
								...
								>
									<span className="text-xl">👎</span>
									<span className="text-[10px]">
										Down ({getSampleCount(GestureType.THUMBS_DOWN)}/
										{SAMPLES_NEEDED})
									</span>
								</button>
							</div>
						...
						<div className="pt-2 border-t border-slate-700">
							<div className="text-xs text-gray-400 mb-1">
								Training Progress: {getTotalSamples()} / {SAMPLES_NEEDED * 3}{" "}
								samples
							</div>
							<div className="h-2 bg-slate-700 rounded-full overflow-hidden">
								<div
									...
									style={{
										width: `${(getTotalSamples() / (SAMPLES_NEEDED * 3)) * 100}%`,
									}}
								/>
							</div>
...
function detectGestureSimple(
	keypoints: any[],
	curls: FingerCurlData,
): GestureType {
